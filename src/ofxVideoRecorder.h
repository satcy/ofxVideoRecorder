...
public:
    ...
    bool setup(string fname, int w, int h, float fps, string audio_file_path, int sampleRate=0, int channels=0, bool sysClockSync=false, bool silent=false);
    bool setupCustomOutput(int w, int h, float fps, string audio_file_path, int sampleRate, int channels, string outputString, bool sysClockSync=false, bool silent=false);
